class Solution:
    def sortColors(self, nums: List[int]) -> None:
        ...
        # ---------- This is a two pointer pattern where we have to sort the array in-place, and we have certain number of categories to sort them into.----------Pattern D - 2 ended partition. (Because there are 2 pointers which mark the end of low and high). The low pointer will tell you where the next low element will go and high will keep track where high value will go.
        low = 0
        high = len(nums) - 1
        mid = 0
        while mid <= high:
